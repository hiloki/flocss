...
운영중에 CSS가 망가지는것은, 섵부른 캐스팅과 셀렉터의 관리에 원인이 있는게 많습니다.
...
왜냐면, 그 레이어에 의한 특정이 모듈에 존재하는것 없이, 모듈로써 독립해 재사용 되어야 하고, 혼재시킴으로써 다른 개발자가 예상하지 않은 동작이 되면 안되기 때문입니다. 
